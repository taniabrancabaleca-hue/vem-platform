            ...
            {pedidos.length > 0 ? pedidos.map((p: any) => (
              <tr key={p.id}>
                <td>
                  ...
                  {p.urgente && (
                    <span className="badge badge-urgente" style={{ marginLeft: 6, fontSize: 10 }}>Urgente</span>
                  )}
                </td>
                <td style={{ fontWeight: 500, fontSize: 13 }}>{p.utente?.nome ?? '—'}</td>
                <td style={{ color: '#6b7280', fontSize: 13 }}>{p.instituicao?.nome ?? '—'}</td>
                <td style={{ color: '#6b7280', fontSize: 13 }}>
                  {SERVICO_LABEL[p.tipo_servico] ?? p.tipo_servico}
                </td>
                <td style={{ color: '#6b7280', fontSize: 12 }}>
                  {p.data_servico
                    ? new Date(p.data_servico).toLocaleDateString('pt-PT', { day: '2-digit', month: '2-digit', year: 'numeric' })
                    : '—'}
                </td>
