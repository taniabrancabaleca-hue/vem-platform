...
export default async function ParceiroPage() {
  ...
  const { data: pedidos } = await supabase
    .from('pedidos')
    .select('id, codigo, servico, data_pedido, estado, urgente')
    ...
    <div className="fade-in">
      ...
      <div className="mobile-only" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {(pedidos ?? []).length > 0 ? (pedidos ?? []).map((p: any) => (
          <div key={p.id} style={{ background: 'white', borderRadius: 12, border: '1px solid #e5e7eb', padding: 16 }}>
            ...
            <p style={{ margin: '0 0 4px', fontSize: 13, color: '#374151' }}>{SERVICO_LABEL[p.servico] ?? p.servico ?? '—'}</p>
      ...
      <div className="data-card desktop-only">
        <table className="vem-table">
          <thead>
            <tr>
              <th>Código</th><th>Serviço</th><th>Data</th><th>Estado</th>
            </tr>
          </thead>
          <tbody>
            {(pedidos ?? []).length > 0 ? (pedidos ?? []).map((p: any) => (
              ...
              <tr><td colSpan={4} style={{ textAlign: 'center', color: '#9ca3af', padding: 48 }}>Nenhum pedido ainda</td></tr>
            )}
          </tbody>
        </table>
      </div>
